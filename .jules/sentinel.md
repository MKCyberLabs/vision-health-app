...
## 2024-10-27 - Add Timeout to External API Calls
**Vulnerability:** The Node.js gateway was making un-timeouted `fetch` requests to the internal Python backend API. If the backend became unresponsive or hung (e.g. during heavy load or a DoS attempt), the Node.js request would hang indefinitely, potentially leading to resource exhaustion (connections, memory) and a cascading denial of service.
**Learning:** Native Node.js `fetch` does not have a default timeout. Unbounded external requests in a server environment represent a significant availability risk and can easily lead to cascading failures across microservices.
**Prevention:** Always include an explicit timeout (e.g., using `AbortSignal.timeout(ms)`) for all server-to-server network requests to ensure they fail fast and release resources if the downstream service is unresponsive.
