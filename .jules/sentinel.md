...
## 2026-08-28 - Missing Timeout on Downstream Fetch Requests
...
## 2024-10-28 - Rate Limit Bypass via Global Path Normalization
**Vulnerability:** The global Express middleware for rate limiting relied on manual path normalization (`req.path.toLowerCase().replace(/\/+$/, '')`) and a global `app.use` instead of applying middleware directly to route definitions. While the normalization attempted to handle casing and trailing slashes, using global path matching for route-specific security controls is fragile and can lead to bypasses if not perfectly aligned with Express's internal router.
**Learning:** Default Express route handlers match flexibly. Applying security middleware directly to the route definitions (e.g., `app.post('/route', middleware, handler)`) ensures the middleware inherently benefits from Express's robust built-in routing logic, preventing bypass vulnerabilities caused by manual path string discrepancies.
**Prevention:** Always apply route-specific security middleware (like rate limiters) directly to the intended route definitions rather than using global `app.use` with manual path matching logic.
