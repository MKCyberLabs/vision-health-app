...
## 2024-10-27 - Rate Limiter Bypass via Path Routing Mismatch
**Vulnerability:** The Express.js rate-limiting middleware in `index.js` was relying on strict equality path checks like `req.path.toLowerCase().replace(/\/+$/, '') === '/analyze'`. This approach incorrectly handled double leading slashes. Attackers could bypass the rate limiter entirely by requesting `//analyze`, which bypassed the global custom middleware check but was successfully routed to the endpoint handler by Express, leading to an effective DoS bypass.
**Learning:** Custom path normalization in global middleware is fragile and error-prone compared to Express's native route matching engine.
**Prevention:** Never rely on manual path normalization in custom global middleware (e.g., `app.use(req.path === '/route')`). Instead, extract security middleware into standalone functions and apply them directly to target route handlers (e.g., `app.post('/analyze', rateLimiter, handler)`) to securely bind the logic to the framework's native matching capabilities.
