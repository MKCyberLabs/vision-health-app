...
## 2026-07-04 - Denial of Service via Process Leakage
**Vulnerability:** In `gemini-api/app.py`, when a `pexpect.TIMEOUT` or generic `Exception` occurred during the execution of external CLI processes (`handle_cli_interaction` and `reply_gemini`), the `pexpect.spawn` child process was removed from `active_sessions` but not explicitly terminated. This caused a process leak, leaving lingering unmanaged `agy` processes running in the background, leading to resource exhaustion (Memory and PID leakage) and potential Denial of Service (DoS).
**Learning:** Removing a reference to a process object (like deleting an entry from `active_sessions`) does not automatically terminate the underlying OS process. If an exception prevents the natural conclusion of the process, it will remain orphaned.
**Prevention:** Always ensure child processes spawned dynamically are explicitly terminated or closed (e.g., using `child.close(force=True)` or `child.terminate(force=True)`) during error handling or cleanup steps to prevent resource leakage. Wrap cleanup code in try-except blocks to avoid crashing the error handlers themselves.
