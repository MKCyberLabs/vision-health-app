...
## 2024-11-20 - Orphaned pexpect processes leading to DoS
**Vulnerability:** The backend spawned `pexpect` CLI processes to handle requests, but failed to explicitly terminate the `child` process (`child.close(force=True)`) in exception handlers and timeouts. If an error occurred or the request timed out, the spawned process would remain running in the background as an orphaned process, consuming system resources and eventually leading to a Denial of Service (DoS).
**Learning:** Tools like `pexpect.spawn` or `subprocess.Popen` create child processes that continue executing even if the parent thread encounters an exception or timeout. Relying on garbage collection or standard application termination is insufficient for long-running services.
**Prevention:** Always ensure explicit termination (e.g., using `child.close(force=True)`) of child processes in exception handlers (`except Exception as e:`) and timeout blocks (`except pexpect.TIMEOUT:`) to prevent resource exhaustion vulnerabilities via orphaned processes. Verify the process is still running (`child.isalive()`) before attempting to close it.
