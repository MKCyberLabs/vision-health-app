...
## 2024-08-01 - Prevent Indefinite Hanging on Fetch Requests
**Vulnerability:** The Node.js Express server was making internal `fetch` requests to a downstream Flask backend API without specifying a timeout. If the backend became unresponsive or dropped packets silently, the `fetch` promises would hang indefinitely. This could easily lead to socket and memory exhaustion (Denial of Service) when multiple requests piled up.
**Learning:** Native Node.js `fetch` (and many HTTP clients) do not have a default timeout. Server-to-server requests must explicitly configure timeouts to ensure the system can fail fast and release resources if a dependency goes down.
**Prevention:** Always use `AbortSignal.timeout(ms)` when making network requests (e.g., `fetch(url, { signal: AbortSignal.timeout(300000) })`) to prevent silent resource leaks during outages.
