...
## 2024-10-27 - Server-to-Server Fetch Timeouts (DoS Prevention)
**Vulnerability:** The Node.js gateway made requests to the Flask backend using native `fetch` without any configured timeout. If the backend became unresponsive or experienced high latency, the Node.js server would hang indefinitely waiting for a response. An attacker could exploit this by making concurrent requests that trigger the backend calls, rapidly exhausting the Express server's connection pool or available memory, leading to a complete Denial of Service.
**Learning:** Native Node.js `fetch` (unlike some older request libraries) does not have a default timeout. It will wait forever for the server to close the connection or respond. In a microservices architecture, this can cause a cascading failure where a slowed internal service brings down the public-facing gateway.
**Prevention:** Always include an `AbortSignal.timeout(ms)` in the options object when making server-to-server requests using native `fetch` to ensure connections are forcefully closed if the downstream service hangs.
